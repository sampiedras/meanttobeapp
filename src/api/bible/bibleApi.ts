...
export const bibleApi = createApi({
  ...
  endpoints: build => ({
    getBooksByBibleId: build.query<BooksEntity[], void>({
      query: () => `${apiBase.endpoints.bibles}/de4e12af7f28f599-01/books`,
      transformResponse: (response: {data: BooksEntity[]}) => response.data,
    }),
    getBooksById: build.query<BooksEntity, string>({
      query: (id) => `${apiBase.endpoints.bibles}/de4e12af7f28f599-01/books/${id}`,
    }),
    getChapterByBookId: build.query<ChapterEntity[], string>({
      query: bookId =>
        `${apiBase.endpoints.bibles}/de4e12af7f28f599-01/books/${bookId}/chapters`,
      transformResponse: (response: {data: ChapterEntity[]}) => response.data,
    }),
    getChapterById: build.query<ChapterEntity, string>({
      query: (id) =>
        `${apiBase.endpoints.bibles}/de4e12af7f28f599-01/chapters/${id}`,
    }),
    getVerseByBookId: build.query<VerseEntity[], string>({
      query: chapterId =>
        `${apiBase.endpoints.bibles}/de4e12af7f28f599-01/chapters/${chapterId}/verses`,
      transformResponse: (response: {data: VerseEntity[]}) => response.data,
    }),
    getVerseById: build.query<VerseEntity, string>({
      query: (id) =>
        `${apiBase.endpoints.bibles}/de4e12af7f28f599-01/verses/${id}`,
    }),
    getPassagesByOrigin: build.query<any, string>({
      query: origin =>
        `${apiBase.endpoints.bibles}/de4e12af7f28f599-01/passages/${origin}?content-type=text&include-notes=false&include-titles=true&include-chapter-numbers=false&include-verse-numbers=true&include-verse-spans=false&use-org-id=false`,
      transformResponse: (response: {data: any}) => response.data,
    }),
    getVerseReferenceById: build.query<VerseEntity, string>({
      query: verseId =>
        `${apiBase.endpoints.bibles}/de4e12af7f28f599-01/verses/${verseId}`,
      transformResponse: (response: {data: VerseEntity}) => response.data,
    }),
  }),
});
